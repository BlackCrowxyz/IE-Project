    ...
    <div class="col-sm-12 rounded-right bg-white">
      <div class="signup">
        <div class="container text-dark">
          <form id="edit-profile" class="py-5" action="" novalidate @submit.prevent>
            <span class="text-center font-weight-bold h5">
              صفحه ویرایش پروفایل
            </span>
            ...
            <button @click="updateUser" type="submit" class="btn btn-success w-50 shadow1 mt-2">
              اعمال تغییرات
            </button>
            <button @click="this.$router.replace(`/report`)" class="btn btn-danger w-50 shadow1 mt-2" onclick="window.location.href = 'report.html'">
              انصرف
            </button>
...
export default {
  ...
  // beforeDestroy() {
  //   console.log("EditProfileComponent > beforeDestroy() called");
  ...
  methods: {
    updateUser() {
      ...
      for (var i = 0; i < this.users.users.length; i++) {
        if (this.users.users[i].email == this.email && filled) {
          if (this.$parent.currentUser.email == this.email) {
            break; //no problem
          }
      ...
      //adding updated user to database
      if (filled && this.$parent.authenticated) {
        this.getUsers();
        let UpdatedUserObj = {
          "id": this.$parent.currentUser.id,
          "username": this.username,
          "email": this.email,
          "password": this.password,
          "isStudent": this.$parent.currentUser.isStudent,
          "isActive": this.$parent.currentUser.isStudent
        };
        //remove the old user with it's id
        this.users.users.splice(this.$parent.currentUser.id,1, UpdatedUserObj);
        ...
        this.$parent.currentUser = UpdatedUserObj;
    ...
    getUsers(){
      console.log('EditProfileComponent > getPosts() called');
      this.$axios
        .get(usersURL)
        .then(response => (this.users = response.data))
        .catch(error => console.log(error))
